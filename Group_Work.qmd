...
dn_props = final_listing[
    final_listing['amenities'].apply(lambda x: any(req_kw in x.lower() for req_kw in required_keywords) if pd.notnull(x) else False)  # at least one required_keywords
    & final_listing['amenities'].apply(lambda x: any(req_kw in x.lower() for req_kw in required_keywords2) if pd.notnull(x) else False)
    & final_listing ['amenities'].str.contains(optional_keywords, regex=True, flags=re.IGNORECASE, na=False)  # at least one optional_keywords
].copy()
...
scenic_keywords = r'(?=scenic|landmark|iconic|museum|famous|historic|heritage|monument|well-known|popular|kensington palace|buckingham palace|tower|cathedral|gallery|London eye)'
...
filtered_listings_transport = gdf_final_listing[gdf_final_listing['name'].str.contains('|'.join(keywords), case=False, na=False)]
#clip data
gdf_listings_transport_clipped = gpd.clip(gdf_final_listing, london_ward)
gdf_listings_transport_clipped = gdf_listings_transport_clipped.set_crs(epsg=27700)
```

### 7.4.2. Visualise the Data on Map
...
print(ward_transport_final.head())
```

```{python}
# Fill NaN values with 0 for wards with no listings
ward_transport_final['listing_count_x'] = ward_transport_final['listing_count_x'].fillna(0)

# Display results
print(ward_transport_final[['GSS_CODE', 'listing_count_x']].head())

# Visualize listings per ward on the map
fig, ax = plt.subplots(figsize=(12, 8))
ward_transport_final.plot(column='listing_count_x', cmap='OrRd', legend=True, ax=ax, edgecolor='black', alpha=0.7)
ax.set_title("Tube Friendly listings by Ward", fontsize=16)
ax.axis('off')
plt.show()
```

#### 7.4.4. Calculate Spatial Autocorrelation
...
# calculate Moran's I
y = ward_transport_final['listing_count_x'].values
moran = esda.Moran(y, w)
print(f"Moran's I: {moran.I}")
print(f"p-value: {moran.p_sim}")
```

### 7.4.5. Perform Local Moran I Analysis
...
ax.set_title('Hotspot vs Non-Hotspot Areas for Tube Friendly Properties in London')
handles = [plt.Line2D([0], [0], marker='o', color='w', markerfacecolor='red', markersize=10, label='Hotspot'),
           plt.Line2D([0], [0], marker='o', color='w', markerfacecolor='orange', markersize=10, label='Non-Hotspot')]
...
# Step 1: Define more precise keywords for Style-Specific Properties
...
ss_props = final_listing[
    final_listing['description'].apply(lambda x: any(req_kw in x.lower() for req_kw in style_keywords_rustic.split("|")) if pd.notnull(x) else False)
    | final_listing['description'].apply(lambda x: any(req_kw in x.lower() for req_kw in style_keywords_vintage.split("|")) if pd.notnull(x) else False)
    | final_listing['description'].apply(lambda x: any(req_kw in x.lower() for req_kw in style_keywords_japanese.split("|")) if pd.notnull(x) else False)
].copy()
...
gdf_ss.plot(ax=ax, marker="o", color="blue", markersize=10, alpha=0.7, label="Style-Specific Properties")
...
## 3. Calculate density 
london_ward["SS_Density"] = london_ward["SS_Count"] / (london_ward["listing_count"])
