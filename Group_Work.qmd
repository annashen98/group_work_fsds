...
pivot_table = gdf.pivot_table(index='neighbourhood', columns='room_type', aggfunc='size', fill_value=0)
...
```{python}
# Filter listings for 'Entire apartment'
entire_apartments = gdf[gdf['room_type'] == 'Entire home/apt']

# Group by borough and count the number of entire apartments
borough_counts = entire_apartments.groupby('neighbourhood').size().reset_index(name='count')

# Merge the count data with the borough geometries
merged = boros.merge(borough_counts, left_on='NAME', right_on='neighbourhood')

# Define custom breaks
breaks = [206, 894, 1769, 3453, 5362, 8487]
labels = ['206-894', '895-1769', '1770-3453', '3454-5362', '5363-8487']

# Create a new column for the discrete categories
merged['count_bins'] = pd.cut(merged['count'], bins=breaks, labels=labels, include_lowest=True)

# Plot the data with custom breaks as discrete categories
fig, ax = plt.subplots(1, 1, figsize=(10, 8))
merged.plot(column='count_bins', cmap='viridis', legend=True, ax=ax)

plt.title('Number of Entire Apartments per Borough (Discrete Categories)')
plt.show()

# Additional plot: Bar plot of number of entire apartments per borough, sorted from highest to lowest
borough_counts_sorted = borough_counts.sort_values(by='count', ascending=False)
fig, ax2 = plt.subplots(figsize=(8, 3))
ax2.bar(borough_counts_sorted['neighbourhood'], borough_counts_sorted['count'], color='skyblue', edgecolor='black')
