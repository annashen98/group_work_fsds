...
The borough with the highest number of entire home or apartment Airbnb listing is Borough of Wesminster, while Havering, Bexley, and Sutton are the borough with lowest number of Airbnb listing as entire home/apartment. Airbnb listing of entire home and apartment are concentrated in central London areas.
...
gdf_sv.plot(ax=ax, marker="o", color="orchid", markersize=8, alpha=0.5, label="Scenic View Properties")
...
```{python}
# 1. calculate the total number of Scenic View Properties in London
...
# 3. Calculate the count of DN points within each ward
...
### 7.3.3 spatial analysis for Scenic View Properties
...
Based on Local Moran's I analysis, the significant hotspot of Airbnb that advertised as having great scenic view are concentrated in Central London.

### **7.4 Close To Tube Properties**

### 7.4.1 filter Close To Tube Properties based on name
...
# draw Scenic View Properties points
gdf_ctt.plot(ax=ax, marker="o", color="orchid", markersize=8, alpha=0.5, label="Scenic View Properties")
...
Based on Local Moran's I analysis, the significant hotspot of Airbnb that advertised as close to tube are concentrated in Central London. While we can see some wards that a bit separated from central London also has significant hotspot.
...
There are only few Japanese style properties while most style-specific properties are dominated with Rustic and Vintage style.
...
plt.barh(list(datasets.keys()), avg_occupancy_rates, color='skyblue')
plt.xlabel('Average Occupancy Rate (Days)')
plt.ylabel('Listing Type')
plt.title('Average Occupancy Rate for Different Listings')
plt.tight_layout()
plt.show()
```

The highest listing type on the average occupancy rate is the scenic-iconic listings while the style-specific listings tend to have lower occupancy rate.
...
plt.figure(figsize=(10, 6))
plt.barh(dataset_names, revenues, color='skyblue')
plt.xlabel('Average Revenue')
plt.ylabel('Listing Type')
plt.title('Average Revenue for Different Listings')
plt.tight_layout()  # Adjust layout to avoid overlap
plt.show()
```

The average revenue is the highest for Digital Nomad type listings while transport-related listings tend to be lowest.
