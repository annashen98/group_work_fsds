...
bib_url = "https://raw.githubusercontent.com/annashen98/group_work_fsds/65928e9bf22ec3080a250419132bdd9fd2e5c288/references.bib"
csl_url = "https://raw.githubusercontent.com/annashen98/group_work_fsds/65928e9bf22ec3080a250419132bdd9fd2e5c288/harvard-cite-them-right.csl"
...
These ethical considerations are addressed from three perspectives: information sources, location information, and neighborhood names. No private information is used in InsideAirbnb data. InsideAirbnb ensures that no private details such as names, photographs, or reviews are used inappropriately, as they are all publicly accessible on the platform. The location data for listings is anonymized by Airbnb, with coordinates scattered within a 0-450 feet (150 meters) range of the actual address to protect privacy. Furthermore, neighborhood names are excluded due to inaccuracies in Airbnb’s naming system, and geographic coordinates are used in conjunction with official city boundaries for more accurate analysis. The site claims "fair use" of this public data, ensuring it serves a non-commercial purpose aimed at public analysis, discussion, and community benefit. 
...
gdf_dn.plot(ax=ax, marker="o", color="orchid", markersize=8, alpha=0.5, label="Digital Nomad Properties")
...
While digital nomad theme properties can be found almost anywhere in London, most of them are concentrated in Central London areas.
...
```{python}
# Get the number of properties in the filtered DataFrame
num_row = len(scenic_landmark)
print(num_row)
```
...
gdf_sv.plot(ax=ax, marker="o", color="orchid", markersize=8, alpha=0.5, label="Scenic-Iconic View Properties")

# add title and legend
plt.title("Scenic-Iconic View Properties in London", fontsize=16)
plt.legend()
...
Based on the map, most of Airbnb that advertised as having great scenic view are concentrated in Central London.

### 7.3.2 Calculate the Dnsity of Scenic-Iconic View Properties
...
# draw Scenic View Properties points
gdf_ctt.plot(ax=ax, marker="o", color="orchid", markersize=8, alpha=0.5, label="Close to Tube Properties")

# add title and legend
plt.title("Close To Tube Properties in London", fontsize=16)
plt.legend()
...
Based on the map, most of Airbnb that advertised as having good tube connectivity are concentrated in Central London.
...
# 4. Inspect the results
print(london_ward[["GSS_CODE", "CTT_Count", "CTT_Density"]].head())

# 5.Plot density distribution
fig, ax = plt.subplots(figsize=(8, 8))
london_ward.plot(column="CTT_Density", ax=ax, cmap="coolwarm", legend=True, 
                 legend_kwds={"label": "Close To Tube Properties Density"})
plt.title("Close To Tube Properties Density in London Wards", fontsize=16)
plt.show()
```
...
ax.set_title('Hotspot vs Non-Hotspot Areas for Close to Tube Properties Density')
handles = [plt.Line2D([0], [0], marker='o', color='w', markerfacecolor='red', markersize=10, label='Hotspot'),
           plt.Line2D([0], [0], marker='o', color='w', markerfacecolor='orange', markersize=10, label='Non-Hotspot')]
...
print(ss_props[['description', 'Style']].head())  
```
...
# draw Style-Specific Properties points
for style, color in style_colors.items():
    subset = gdf_ss[gdf_ss['Style'] == style]  # filter subset of Style
    subset.plot(
        ax=ax,
        marker="o",
        color=color,
        markersize=8,
        alpha=0.7,
        label=style.capitalize()  # use capital as lable
    )

# add title and legend
plt.title("Style-Specific Properties in London", fontsize=16)
plt.legend()
...
There are only few Japanese style properties while most style-specific properties are dominated by Rustic and Vintage style in central areas.
...
# 4. Inspect the results
print(london_ward[["GSS_CODE", "SS_Count", "SS_Density"]].head())

# 5.Plot density distribution
fig, ax = plt.subplots(figsize=(8, 8))
london_ward.plot(column="SS_Density", ax=ax, cmap="coolwarm", legend=True, 
                 legend_kwds={"label": "Style-Specific Properties Density"})
plt.title("Style-Specific Properties Density in London Wards", fontsize=16)
plt.show()
```
...
ax.set_title('Hotspot vs Non-Hotspot Areas for Style-Specific Properties Density')
handles = [plt.Line2D([0], [0], marker='o', color='w', markerfacecolor='red', markersize=10, label='Hotspot'),
           plt.Line2D([0], [0], marker='o', color='w', markerfacecolor='orange', markersize=10, label='Non-Hotspot')]
...
#### 7.7.1. Conclusions
London's 90-day short-term rental regulation limits market flexibility, but it also presents an opportunity for more refined policies and better resource management through a deeper understanding of property distribution and branding potential. The Mayor of London could introduce flexible policies that account for seasonal demand and the proximity to London’s top tourist attractions, such as relaxing the 90-day cap in central areas during peak tourist seasons to boost economic activity. Branding efforts could align with these regulations by highlighting unique landmark experiences to attract premium short-term guests. Additionally, brands in central London could apply for long-term permits, shifting to mid- or long-term rental markets to meet the growing demand from digital nomads seeking extended stays.

#### 7.7.2. Limitation

The Natural Language Processing have many limitations, including the words and phrases that chosen for the analysis and how the data acquired. Some data may not be appropriate since the word that contain on the data may not represent our intended classification. 
